...
import { deleteUserHashtags, getUserHashtags, updateUserInterests, uploadHashtag } from '../views/hashtags';
...
router.post('/:postId/create', checkAuthentication, async (req, res, next) => {
    try {
        ...
        const hashtagsCreation = hashtags.map(hashtag => uploadHashtag(post.id, hashtag));
...
router.post('/interests', checkAuthentication, async (req, res, next) => {
    try {
        ...
        const userInterests = hashtags.map(hashtag => updateUserInterests(userId, hashtag.id));
