...
export const isUserInterestedInHashtag = async (userId: number, hashtagId: number) => {
    const existingRows = await db.select().from(userHashtags).where(
      and(
...
export const updateUserInterests = async (userId: number, hashtagId: number) => {
    const isUserAlreadyInterested = await isUserInterestedInHashtag(userId, hashtagId);
...
export const getUserHashtags = async (userId: number) => {
    const hashtagIds = await db.select().from(userHashtags).where(eq(userHashtags.userId, userId));

    const hashtagsRequests = hashtagIds.map(({ hashtagId }) => getHashtagById(hashtagId));
